...
abstract class QuestionAbstractRequest extends PlatformRequest
{
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'options' => [Rule::requiredIf(in_array($this->input('type'), QuestionType::hasRequiredOptions())), 'array'],
//            'options.*' => ['required', 'string', 'max:255'],
        ];

        foreach ($this->input('options') ?? [] as $i => $option) {
            $rules['options.'.$i.'.ar'] = [Rule::requiredIf(!$this->input('options.'.$i.'.en')), 'string', 'max:255'];
            $rules['options.'.$i.'.en'] = [Rule::requiredIf(!$this->input('options.'.$i.'.ar')), 'string', 'max:255'];
        }

        if ($this->isMethod('PUT')) {
            $rules['existing_options'] = ['required', 'array'];
