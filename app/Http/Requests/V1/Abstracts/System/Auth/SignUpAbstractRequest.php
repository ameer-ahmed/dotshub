...
abstract class SignUpAbstractRequest extends PlatformRequest
{
    ...
    {
        $subdomain = (string)$this->input('tenant_subdomain', '');
        $subdomain = $this->normalizeSubdomain($subdomain);

        if ($subdomain === '' || str_contains($subdomain, '.')) {
            throw ValidationException::withMessages([
                'tenant_subdomain' => __('validation.regex', ['attribute' => 'tenant_subdomain']),
            ]);
        }
    ...
    public function rules(): array
    {
        return [
            ...
            'tenant_description' => ['required', 'string', 'min:100'],
            'tenant_subdomain' => ['required', 'string', 'max:50', 'regex:/^(?!-)[A-Za-z0-9-]+(?<!-)$/'],
            'tenant_domain' => ['present', 'string'],
        ];
    }
    ...
    private function ensureUniqueOrFail(string $subdomain): void
    {
        ...
        if (!$isUnique) {
            throw ValidationException::withMessages([
                'tenant_subdomain' => __('validation.unique', ['attribute' => 'tenant_subdomain']),
            ]);
        }
    }
}
