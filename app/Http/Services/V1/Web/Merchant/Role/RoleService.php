...
use App\Http\Resources\V1\Web\Merchant\Role\RoleResource;
...
class RoleService extends RoleAbstractService
{
    ...
    public function update(RoleRequest $request, $id)
    {
        ...
        if (
            (!$user->hasRole('merchant_admin') && $user->id !== $role->created_by)
    ...
    public function delete($id)
    {
        ...
        if (!$user->hasRole('merchant_admin') && $user->id !== $role->created_by) {
            return Responser::fail(
                Response::HTTP_FORBIDDEN,
                __('Forbidden action. If the problem persists, please contact technical support.')
            );
